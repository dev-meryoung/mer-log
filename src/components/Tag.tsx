...
const Tag: React.FC<TagProps> = ({ label }) => {
  ...
  return (
    <button
      type='button'
      className={`flex justify-center items-center text-sm md:text-base gap-1 px-2.5 py-1.5 rounded-3xl ${active ? 'bg-secondary text-text-dark dark:bg-blue-400' : 'bg-gray-200 dark:bg-gray-500 dark:text-text-dark'}
      `}
      onClick={handleClick}
    >
      {label}
